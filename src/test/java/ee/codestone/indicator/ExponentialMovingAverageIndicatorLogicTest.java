 ...
 */
public class ExponentialMovingAverageIndicatorLogicTest extends IndicatorLogicTest {

	public static final String EMA_TEST_CSV = "ema.csv";

	@Test
	public void testCalculateSMA() throws Exception {
		IndicatorCalculator calculator = prepareCalculator(new BigDecimal(10));

		Collection<PriceData> testData = prepareTestData(EMA_TEST_CSV);
		List<BigDecimal> expectedResults = prepareExpectations(EMA_TEST_CSV);

		List<ChartData> results = executeCalculation(calculator, testData);

		for (int i = 0; i < results.size(); i++) {
			assertEquals(expectedResults.get(i), findIndicatorValues(results.get(i), IndicatorType.EMA).get("value"));
		}
	}
	...
	private IndicatorCalculator prepareCalculator(BigDecimal period) {
		...
		indicators.add(new IndicatorDefinition(IndicatorType.EMA, params, null));
